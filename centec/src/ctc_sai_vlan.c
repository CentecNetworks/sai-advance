...
ctc_sai_vlan_set_info(sai_object_key_t *key, const sai_attribute_t* attr)
{
    ...
    case SAI_VLAN_ATTR_BROADCAST_FLOOD_CONTROL_TYPE:
        {
            ...
            _ctc_sai_vlan_set_unknown_pkt_action(lchip, p_db_vlan->user_vlanptr, p_switch_master->fdb_miss_action[(attr->id - SAI_VLAN_ATTR_UNKNOWN_UNICAST_FLOOD_CONTROL_TYPE) / step], p_db_vlan->ukwn_flood_ctr[(attr->id - SAI_VLAN_ATTR_UNKNOWN_UNICAST_FLOOD_CONTROL_TYPE) / step],
    ...
    case SAI_VLAN_ATTR_META_DATA:
        CTC_SAI_CTC_ERROR_RETURN(ctcs_vlan_set_direction_property(lchip, vlan_ptr, CTC_VLAN_DIR_PROP_ACL_CLASSID, CTC_INGRESS,
                                                                  CTC_SAI_META_DATA_SAI_TO_CTC(attr->value.u32)));
        CTC_SAI_CTC_ERROR_RETURN(ctcs_vlan_set_direction_property(lchip, vlan_ptr, CTC_VLAN_DIR_PROP_ACL_CLASSID_1, CTC_INGRESS,
                                                                  CTC_SAI_META_DATA_SAI_TO_CTC(attr->value.u32)));
        CTC_SAI_CTC_ERROR_RETURN(ctcs_vlan_set_direction_property(lchip, vlan_ptr, CTC_VLAN_DIR_PROP_ACL_CLASSID_2, CTC_INGRESS,
                                                                  CTC_SAI_META_DATA_SAI_TO_CTC(attr->value.u32)));
        CTC_SAI_CTC_ERROR_RETURN(ctcs_vlan_set_direction_property(lchip, vlan_ptr, CTC_VLAN_DIR_PROP_ACL_CLASSID_3, CTC_INGRESS,
                                                                  CTC_SAI_META_DATA_SAI_TO_CTC(attr->value.u32)));
        break;
...
ctc_sai_vlan_get_info(sai_object_key_t *key, sai_attribute_t* attr, uint32 attr_idx)
{
    ...
    case SAI_VLAN_ATTR_CUSTOM_IGMP_SNOOPING_ENABLE:
        ...
        break;
    case SAI_VLAN_ATTR_META_DATA:
        CTC_SAI_CTC_ERROR_RETURN(ctcs_vlan_get_direction_property(lchip, vlan_ptr, CTC_VLAN_DIR_PROP_ACL_CLASSID, CTC_INGRESS, &u_value_32));
        attr->value.u32 = CTC_SAI_META_DATA_CTC_TO_SAI(u_value_32);
        break;
    case SAI_VLAN_ATTR_CUSTOM_STATS_ENABLE:
                                      ...
                                      const sai_attribute_t *attr_list)
{
    ...
    *sai_vlan_id = ctc_sai_create_object_id(SAI_OBJECT_TYPE_VLAN, lchip, 0, 0, (uint32)vlan.user_vlanptr);
    ...
    if (status == SAI_STATUS_SUCCESS )
    {
        CTC_SAI_CTC_ERROR_GOTO(ctcs_vlan_set_direction_property(lchip, vlan.user_vlanptr, CTC_VLAN_DIR_PROP_ACL_CLASSID, CTC_INGRESS,
                                                                CTC_SAI_META_DATA_SAI_TO_CTC(attr_value->u32)), status, error4);
        CTC_SAI_CTC_ERROR_GOTO(ctcs_vlan_set_direction_property(lchip, vlan.user_vlanptr, CTC_VLAN_DIR_PROP_ACL_CLASSID_1, CTC_INGRESS,
                                                                CTC_SAI_META_DATA_SAI_TO_CTC(attr_value->u32)), status, error4);
        CTC_SAI_CTC_ERROR_GOTO(ctcs_vlan_set_direction_property(lchip, vlan.user_vlanptr, CTC_VLAN_DIR_PROP_ACL_CLASSID_2, CTC_INGRESS,
                                                                CTC_SAI_META_DATA_SAI_TO_CTC(attr_value->u32)), status, error4);
        CTC_SAI_CTC_ERROR_GOTO(ctcs_vlan_set_direction_property(lchip, vlan.user_vlanptr, CTC_VLAN_DIR_PROP_ACL_CLASSID_3, CTC_INGRESS,
                                                                CTC_SAI_META_DATA_SAI_TO_CTC(attr_value->u32)), status, error4);
    }

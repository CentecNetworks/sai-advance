...
public:
  ...
  void sai_attributes_u16_to_sai_thrift_list(sai_attribute_t *sai_attrs, uint32_t count, std::vector<sai_thrift_attribute_t> & thrift_attr_list) {
...
void sai_thrift_get_vlan_attribute(sai_thrift_attribute_list_t& thrift_attr_list, const sai_thrift_object_id_t vlan_id)
{
  ...
  void sai_thrift_remove_vlan_members(sai_thrift_status_list_t & ret, const std::vector<sai_thrift_object_id_t> & thrift_object_id_list, int8_t mode) {
  ...
  void sai_thrift_get_routes_attribute(sai_thrift_attribute_list_t& thrift_attr_list, const std::vector<sai_thrift_route_entry_t> & thrift_route_entry_list, const int8_t mode)
  {
...
void sai_thrift_get_lag_attribute(sai_thrift_attribute_list_t& thrift_attr_list, const sai_thrift_object_id_t lag_id)
{
  ...
  void sai_thrift_remove_stp_ports(sai_thrift_status_list_t & ret, const std::vector<sai_thrift_object_id_t> & thrift_object_id_list, int8_t mode) {
...
void sai_thrift_get_udf_group_attribute(sai_thrift_attribute_list_t& thrift_attr_list, const sai_thrift_object_id_t udf_group_id)
{
  ...
  void sai_thrift_parse_mirror_session_attributes(const std::vector<sai_thrift_attribute_t> &thrift_attr_list, sai_attribute_t *attr_list, sai_object_id_t **buffer_profile_list)
  {
      std::vector < sai_thrift_attribute_t > ::const_iterator it = thrift_attr_list.begin();
      sai_thrift_attribute_t attribute;
      for (uint32_t i = 0; i < thrift_attr_list.size(); i++, it++)
  ...
  void sai_thrift_parse_twamp_session_attributes(const std::vector<sai_thrift_attribute_t> &thrift_attr_list, sai_attribute_t *attr_list)
  {
      std::vector < sai_thrift_attribute_t > ::const_iterator it = thrift_attr_list.begin();
      sai_thrift_attribute_t attribute;
      for (uint32_t i = 0; i < thrift_attr_list.size(); i++, it++)
  ...
  void sai_thrift_get_twamp_attribute(sai_thrift_attribute_list_t& thrift_attr_list, const sai_thrift_object_id_t twamp_session_id)
  {
      ...
      attr[0].id = SAI_TWAMP_SESSION_ATTR_PORT;
      attr[1].id = SAI_TWAMP_SESSION_ATTR_SESSION_ROLE;
	  attr[2].id = SAI_TWAMP_SESSION_ATTR_UDP_SRC_PORT;
	  attr[3].id = SAI_TWAMP_SESSION_ATTR_UDP_DST_PORT;
      attr[4].id = SAI_TWAMP_SESSION_ATTR_SRC_IP;
	  attr[5].id = SAI_TWAMP_SESSION_ATTR_DST_IP;
	  attr[6].id = SAI_TWAMP_SESSION_ATTR_TC;
      attr[7].id = SAI_TWAMP_SESSION_ATTR_VPN_VIRTUAL_ROUTE;
	  attr[8].id = SAI_TWAMP_SESSION_ATTR_TWAMP_ENCAPSULATION_TYPE;
	  attr[9].id = SAI_TWAMP_SESSION_ATTR_SESSION_ENABLE_TRANSMIT;
	  attr[10].id = SAI_TWAMP_SESSION_ATTR_HW_LOOKUP_VALID;
	  attr[11].id = SAI_TWAMP_SESSION_ATTR_PADDING_LENGTH;
	  attr[12].id = SAI_TWAMP_SESSION_ATTR_STATE;
	  attr[13].id = SAI_TWAMP_SESSION_ATTR_AUTH_MODE;
	  attr[14].id = SAI_TWAMP_SESSION_ATTR_NEXT_HOP_ID;
	  attr[15].id = SAI_TWAMP_SESSION_ATTR_TX_PKT_PERIOD;
	  attr[16].id = SAI_TWAMP_SESSION_ATTR_TX_RATE;
	  attr[17].id = SAI_TWAMP_SESSION_ATTR_TX_PKT_CNT;
	  attr[18].id = SAI_TWAMP_SESSION_ATTR_TX_PKT_DURATION;
	  attr[19].id = SAI_TWAMP_SESSION_ATTR_MODE;
      attr_count = 20;

      status = twamp_api->get_twamp_session_attribute(twamp_session_id, attr_count, attr);
      if (status != SAI_STATUS_SUCCESS) {
      ...
      thrift_attr.id        = attr[4].id;
      thrift_attr.value.ipaddr.addr_family = attr[4].value.ipaddr.addr_family;
      if (SAI_IP_ADDR_FAMILY_IPV4 == thrift_attr.value.ipaddr.addr_family)
      {
          thrift_attr.value.ipaddr.addr.ip4 = sai_thrift_v4_ip_to_string(attr[4].value.ipaddr.addr.ip4);
      }
      else
      {
      ...
      thrift_attr.id        = attr[5].id;
      thrift_attr.value.ipaddr.addr_family = attr[5].value.ipaddr.addr_family;
      if (SAI_IP_ADDR_FAMILY_IPV4 == thrift_attr.value.ipaddr.addr_family)
      {
          thrift_attr.value.ipaddr.addr.ip4 = sai_thrift_v4_ip_to_string(attr[5].value.ipaddr.addr.ip4);
      }
      else
      {
      ...
      thrift_attr.value.__set_booldata(attr[9].value.booldata);
      thrift_attr_list.attr_list.push_back(thrift_attr);

      thrift_attr.id        = attr[10].id;
      thrift_attr.value.__set_booldata(attr[10].value.booldata);
      thrift_attr_list.attr_list.push_back(thrift_attr);
      ...
      thrift_attr.id        = attr[15].id;
      thrift_attr.value.__set_u32(attr[15].value.u32);
      thrift_attr_list.attr_list.push_back(thrift_attr);

      thrift_attr.id        = attr[16].id;
      thrift_attr.value.__set_u32(attr[16].value.u32);
      thrift_attr_list.attr_list.push_back(thrift_attr);

      thrift_attr.id        = attr[17].id;
      thrift_attr.value.__set_u32(attr[17].value.u32);
      thrift_attr_list.attr_list.push_back(thrift_attr);

      thrift_attr.id        = attr[18].id;
      thrift_attr.value.__set_u64(attr[18].value.u64);
      thrift_attr_list.attr_list.push_back(thrift_attr);

      thrift_attr.id        = attr[19].id;
      thrift_attr.value.__set_u32(attr[19].value.u32);
      thrift_attr_list.attr_list.push_back(thrift_attr);
  }
...
sai_thrift_object_id_t sai_thrift_create_rpf_group_member(const std::vector<sai_thrift_attribute_t> & thrift_attr_list) {
 ...
 sai_thrift_object_id_t sai_thrift_create_debug_counter(const std_sai_thrift_attr_vctr_t &thrift_attr_list)
  ...
  void sai_thrift_get_y1731_session_attribute(sai_thrift_attribute_list_t& thrift_attr_list, const sai_thrift_object_id_t y1731_meg_oid)
  {
      ...
      attr[attr_id].id = SAI_Y1731_SESSION_ATTR_MEG;
      attr_id++;
                                  ...
                                  const int32_t number_of_stats)
  {
    ...
    void sai_thrift_parser_hostif_packet_attributes(const std::vector<sai_thrift_attribute_t> &thrift_attr_list, sai_attribute_t *attr_list)
    {
      if (attr_list == nullptr || thrift_attr_list.empty())
      { SAI_THRIFT_LOG_ERR("Invalid input arguments."); return; }
                                                           ...
                                                           const std::vector<sai_thrift_attribute_t> & thrift_attr_list)
   {
      ...
      status = host_api->send_hostif_packet(thrift_hif_id,  (sai_size_t)packet_data.length(),  (void*)packet, attr_size, attr_list);
      sai_thrift_free_attr(attr_list);
      ...
      for (sai_uint32_t i = 0; i < thrift_attr_list.size(); i++, cit++)
      {
          ...
          switch (attribute.id)
          {
              ...
              case SAI_PTP_DOMAIN_ATTR_ADJUEST_CLOCK_OFFSET:
                  ...
                  attr_list[i].value.timeoffset.type= attribute.value.timeoffset.type;
		    break;
  ...
  void sai_thrift_get_ptp_domain_attribute(sai_thrift_attribute_list_t& thrift_attr_list, const sai_thrift_object_id_t ptp_domain_oid) {
      ...
      attr[attr_id].id = SAI_PTP_DOMAIN_ATTR_ADJUEST_CLOCK_OFFSET;
      attr_id++;
      ...
      thrift_attr.value.timeoffset.type = attr[attr_id].value.timeoffset.type;

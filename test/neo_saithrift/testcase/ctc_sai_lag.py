...
class fun_02_create_max_lag_fn(sai_base_test.ThriftInterfaceDataPlane):
    def runTest(self):
        ...
        lag_list = []
        for i in range(256):
            lag_id = sai_thrift_create_lag(self.client, max_member=4)
...
class fun_03_create_max_lag_member_fn(sai_base_test.ThriftInterfaceDataPlane):
    def runTest(self):
        ...
        lag_list = []
        for i in range(20):
            lag_id = sai_thrift_create_lag(self.client, max_member=100)
            lag_list.append(lag_id)
        #total 2000 lag member
        try:
            lag_id1 = sai_thrift_create_lag(self.client, max_member=100)
            sys_logging("lag1 oid = 0x%x" %lag_id1)
            assert(lag_id1==SAI_NULL_OBJECT_ID)

            #2048-2000=48
            lag_id2 = sai_thrift_create_lag(self.client, max_member=48)
            sys_logging("lag2 oid = 0x%x" %lag_id2)
            assert(lag_id2%0x100000000==2)

            lag_id3 = sai_thrift_create_lag(self.client, max_member=100)
            sys_logging("lag3 oid = 0x%x" %lag_id3)
...
class fun_04_create_5_mode_invalid_param_lag_fn(sai_base_test.ThriftInterfaceDataPlane):
    def runTest(self):
        ...
        lag_id3 = sai_thrift_create_lag(self.client, lag_mode=lag_mode2, max_member=25)
        lag_id4 = sai_thrift_create_lag(self.client, lag_mode=lag_mode3, max_member=255)
        lag_id5 = sai_thrift_create_lag(self.client, lag_mode=lag_mode4, max_member=255)
        ...
        lag_id8 = sai_thrift_create_lag(self.client, lag_mode=lag_mode2, max_member=24)
        lag_id9 = sai_thrift_create_lag(self.client, lag_mode=lag_mode3, max_member=256)
        lag_id10 = sai_thrift_create_lag(self.client, lag_mode=lag_mode4, max_member=256)
        try:
            sys_logging("lag1 oid = 0x%x" %lag_id1)
            assert(lag_id1==SAI_NULL_OBJECT_ID)
            sys_logging("lag2 oid = 0x%x" %lag_id2)
            ...
            assert(lag_id6%0x100000000==2) 
            sys_logging("lag7 oid = 0x%x" %lag_id7)
            assert(lag_id7%0x100000000==2)   
            sys_logging("lag8 oid = 0x%x" %lag_id8)
            assert(lag_id8%0x100000000==2)
            sys_logging("lag9 oid = 0x%x" %lag_id9)
            assert(lag_id9%0x100000000==2)
            sys_logging("lag10 oid = 0x%x" %lag_id10)
            assert(lag_id10%0x100000000==2)
...
class fun_07_set_lag_attr_igs_acl_fn(sai_base_test.ThriftInterfaceDataPlane):
    def runTest(self):
        ...
        try:
            ...
            lag_attr_value = sai_thrift_attribute_value_t(oid=acl_table_id)
            attribute = sai_thrift_attribute_t(id=SAI_LAG_ATTR_INGRESS_ACL, value=lag_attr_value)
            status = self.client.sai_thrift_set_lag_attribute(lag_id1, attribute)
            sys_logging( "status = %d" %status)
            assert (status == SAI_STATUS_SUCCESS)
            ...
            for a in attrs.attr_list:
                if a.id == SAI_LAG_ATTR_INGRESS_ACL:
                    sys_logging("set ingress acl = %d" %acl_table_id)
                    sys_logging("get ingress acl = %d" %a.value.oid)
                    assert(acl_table_id == a.value.oid)
             
        finally:
            sys_logging("======clean up======")
            lag_attr_value = sai_thrift_attribute_value_t(oid=SAI_NULL_OBJECT_ID)
            attribute = sai_thrift_attribute_t(id=SAI_LAG_ATTR_INGRESS_ACL, value=lag_attr_value)
            status = self.client.sai_thrift_set_lag_attribute(lag_id1, attribute)
...
class fun_08_set_lag_attr_pvid_fn(sai_base_test.ThriftInterfaceDataPlane):
    def runTest(self):
        ...
        try:
            ...
            lag_attr_value = sai_thrift_attribute_value_t(u16=port_vlan_id)
            ...
            for a in attrs.attr_list:
                if a.id == SAI_LAG_ATTR_PORT_VLAN_ID:
                    sys_logging("set port vlan id = %d" %port_vlan_id)
                    sys_logging("get port vlan id = %d" %a.value.u16)
                    assert(port_vlan_id == a.value.u16)
...
class fun_09_set_lag_attr_default_cos_fn(sai_base_test.ThriftInterfaceDataPlane):
    def runTest(self):
        ...
        try:
            ...
            for a in attrs.attr_list:
                if a.id == SAI_LAG_ATTR_DEFAULT_VLAN_PRIORITY:
                    sys_logging("set default vlan priority = %d" %default_vlan_cos)
                    sys_logging("get default vlan priority = %d" %a.value.u8)
                    assert(default_vlan_cos == a.value.u8)
...
class fun_10_set_lag_attr_drop_untagged_fn(sai_base_test.ThriftInterfaceDataPlane):
    def runTest(self):
        ...
        try:
            ...
            for a in attrs.attr_list:
                if a.id == SAI_LAG_ATTR_DROP_UNTAGGED:
                    sys_logging("set drrop untagged value = %d" %drop_untagged)
                    sys_logging("get drrop untagged value = %d" %a.value.booldata)
                    assert(drop_untagged == a.value.booldata)
...
class fun_11_set_lag_attr_drop_tagged_fn(sai_base_test.ThriftInterfaceDataPlane):
    def runTest(self):
        ...
        try:
            ...
            for a in attrs.attr_list:
                if a.id == SAI_LAG_ATTR_DROP_TAGGED:
                    sys_logging("set drrop tagged value = %d" %drop_tagged)
                    sys_logging("get drrop tagged value = %d" %a.value.booldata)
                    assert(drop_tagged == a.value.booldata)
...
class fun_13_get_lag_all_attr_fn(sai_base_test.ThriftInterfaceDataPlane):
    def runTest(self):
        ...
        drop_tagged = True
        max_member = 128
        lag_id1 = sai_thrift_create_lag(self.client, lag_mode=lag_mode4, pvid=port_vlan_id, default_vlan_pri=default_cos,drop_untagged=drop_untagged, drop_tagged=drop_tagged, max_member=max_member)
        sys_logging("lag1 oid = 0x%x" %lag_id1)
        try:
            attrs= self.client.sai_thrift_get_lag_attribute(lag_id1)
            sys_logging( "status = %d" %attrs.status)
        ...
        try:
            ...
            for a in attrs.attr_list:
                if a.id == SAI_LAG_ATTR_PORT_LIST:
                    print "get lag member list =",
                    print a.value.objlist.object_id_list
                    assert (a.value.objlist.object_id_list == [520093723, 4815061019, 9110028315]) #[0x1f00001b, 0x11f00001b, 0x21f00001b]
        finally:
            sys_logging("======clean up======")
...
class fun_19_set_lag_member_attribute_igs_disable_fn(sai_base_test.ThriftInterfaceDataPlane):
    def runTest(self):
        ...
        ingress_disable1 = True
        ingress_disable2 = False

        try:
            attr_value = sai_thrift_attribute_value_t(booldata=ingress_disable1)
            attr = sai_thrift_attribute_t(id=SAI_LAG_MEMBER_ATTR_INGRESS_DISABLE, value=attr_value)
            
            status =self.client.sai_thrift_set_lag_member_attribute(lag_member_id1, attr)
            attrs = self.client.sai_thrift_get_lag_member_attribute(lag_member_id1)
            ...
            status =self.client.sai_thrift_set_lag_member_attribute(lag_member_id2, attr)
            attrs = self.client.sai_thrift_get_lag_member_attribute(lag_member_id2)
            sys_logging( "get lag member attribute status = %d" %attrs.status)
            assert (attrs.status == SAI_STATUS_SUCCESS)
            for a in attrs.attr_list:
            ...
            attr_value = sai_thrift_attribute_value_t(booldata=ingress_disable2)
            attr = sai_thrift_attribute_t(id=SAI_LAG_MEMBER_ATTR_INGRESS_DISABLE, value=attr_value)
...
class fun_20_set_lag_member_attribute_egs_disable_fn(sai_base_test.ThriftInterfaceDataPlane):
    def runTest(self):
        ...
        egress_disable1 = True
        egress_disable2 = False

        try:
            attr_value = sai_thrift_attribute_value_t(booldata=egress_disable1)
            ...
            attr_value = sai_thrift_attribute_value_t(booldata=egress_disable2)
            attr = sai_thrift_attribute_t(id=SAI_LAG_MEMBER_ATTR_EGRESS_DISABLE, value=attr_value)
...
class scenario_02_lag_bind_bridge_port_fdb_test(sai_base_test.ThriftInterfaceDataPlane):
    def runTest(self):
        ...
        sai_thrift_create_fdb(self.client, vlan_oid, mac1, lag_oid1, mac_action, is_lag=1)
        sai_thrift_create_fdb(self.client, vlan_oid, mac2, lag_oid2, mac_action, is_lag=1)
        ...
        try:
            ...
            sai_thrift_delete_fdb(self.client, vlan_oid, mac2, lag_oid2)
...
class scenario_04_lag_bind_sub_port_test(sai_base_test.ThriftInterfaceDataPlane):
    def runTest(self):
        ...
        sub_port_id1 = sai_thrift_create_bridge_sub_port(self.client, lag_id1, bridge_id1, vlan_id1, is_lag=1)
        sub_port_id2 = sai_thrift_create_bridge_sub_port(self.client, lag_id1, bridge_id1, vlan_id2, is_lag=1)
        sub_port_id3 = sai_thrift_create_bridge_sub_port(self.client, lag_id2, bridge_id1, vlan_id1, is_lag=1)
...
class scenario_05_lag_bind_sub_port_add_and_remove_test(sai_base_test.ThriftInterfaceDataPlane):
    def runTest(self):
        ...
        sub_port_id1 = sai_thrift_create_bridge_sub_port(self.client, lag_id1, bridge_id1, vlan_id1, is_lag=1)
        sub_port_id2 = sai_thrift_create_bridge_sub_port(self.client, lag_id2, bridge_id1, vlan_id2, is_lag=1)

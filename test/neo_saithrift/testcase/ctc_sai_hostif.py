...
class ControlPlaneBaseTest(sai_base_test.ThriftInterfaceDataPlane):
    ...
    my_ip = "10.0.0.1"
    peer_ip = "10.0.0.0"
    my_ipv6_ip = "1234:5678:9abc:def0:4422:1133:5577:99aa"
    peer_ipv6_ip = "1234:5678:9abc:def0:4422:1133:5577:99a0"
    ...
    def setup_direct_ipv6_route(self, vr_id, rif_id1, ip_addr):

        addr_family = SAI_IP_ADDR_FAMILY_IPV6
        ip_addr1 = ip_addr
        ip_addr1_subnet = ip_addr
        ip_mask1 = 'ffff:ffff:ffff:ffff:ffff:ffff:ffff:fffE'
        sai_thrift_create_route(self.client, vr_id, addr_family, ip_addr1_subnet, ip_mask1, rif_id1)
        route_data=[vr_id, addr_family, ip_addr1_subnet, ip_mask1, rif_id1]
        self.routes.append(route_data)
    ...
    def setup_ip2me_ipv6_route(self, ip_addr):
    ...
    def create_ipv6_routes(self):
        self.setup_ip2me_ipv6_route(self.peeripv6ip)
        vr_id, rif_id = self.setup_test_port_rif(self.test_port_ind)
        self.setup_direct_ipv6_route(vr_id, rif_id, self.peeripv6ip)
        return
    ...
    def setUp(self):
        ...
        #self.dataplane.port_up(0, self.test_port_ind)
        #time.sleep(10)
    ...
    def tearDown(self):
        ...
        for interface,front in interface_to_front_mapping.iteritems():
            ...
            self.hostif_id[front]=0


        #self.dataplane.port_down(0, self.test_port_ind)
    ...
    def copp_test(self, packet, count, send_intf, recv_intf):
        ...
        #old process mark start
        '''
        end_time=datetime.now()
        print recv_intf[1]
        print recv_intf[0]
        total_rcv_pkt_cnt = self.ctc_count_matched_packets(packet, recv_intf[1], recv_intf[0], timeout=5)
    ...
    def one_port_test(self, port_number, packet_count):
        ...
        total_rcv_pkt_cnt, time_delta, time_delta_ms, tx_pps, rx_pps = self.copp_test(packet, packet_count, port_number, (0, 5))
...
class LACPTest(NoPolicyTest):
    ...
    def contruct_packet(self):
        packet = simple_eth_packet(pktlen=50,
...
class ICMPv6Test(NoPolicyTest):
    ...
    def setUp(self):
        ...
        ipv6allnodeip = 'ff02::1'
        #self.setup_ip2me_ipv6_route(ipv6allnodeip)
        vr_id, rif_id = self.setup_test_port_rif(self.test_port_ind)
    ...
    def contruct_packet(self):
        #src_mac = self.src_mac_uc
        packet = simple_icmpv6_packet(eth_dst='33:33:00:00:00:01',
...
class OSPFv2Test(NoPolicyTest):
    ...
    def contruct_packet(self):
        #src_mac = self.src_mac_uc
        packet = simple_ip_packet(eth_dst='e0:00:00:00:00:01',
                         eth_src='00:06:07:08:09:0a',
                         dl_vlan_enable=False,
                         vlan_vid=0,
                         vlan_pcp=0,
...
class OSPFv3Test(NoPolicyTest):
    ...
    def contruct_packet(self):
        #src_mac = self.src_mac_uc
        packet = simple_ipv6_packet(eth_dst='33:33:00:00:00:05',
                         eth_src='00:06:07:08:09:0a',
                         dl_vlan_enable=False,
                         vlan_vid=0,
                         vlan_pcp=0,
...
class ISISBroadcastTest(NoPolicyTest):
    def __init__(self):
        NoPolicyTest.__init__(self)
        return
    ...
    def setUp(self):
        ...
        trap = sai_thrift_create_hostif_trap(client=self.client,
                                             trap_type=SAI_HOSTIF_TRAP_TYPE_CUSTOM_EXCEPTION_ISIS,
                                             packet_action=SAI_PACKET_ACTION_TRAP,
                                             trap_group=trap_group)
        self.traps.append(trap)
        return
    ...
    def contruct_packet(self):
        ...
        src_mac = self.src_mac_uc
        packet = simple_eth_packet(eth_dst='01:80:c2:00:00:15',
                                   eth_src=src_mac,
                                   eth_type=0x55aa)
        return packet
        
class ISISP2PTest(NoPolicyTest):
    def __init__(self):
        NoPolicyTest.__init__(self)
        return
    ...
    def contruct_packet(self):
        ...
        src_mac = self.src_mac_uc
        packet = simple_eth_packet(eth_dst='09:00:2b:00:00:05',
                                   eth_src=src_mac,
                                   eth_type=0x55aa)
        return packet
        
class scenario_01_cpu_tx_routing_test(sai_base_test.ThriftInterfaceDataPlane):
    def runTest(self):
        ...
        try:
            ...
            host_if_tx_type = SAI_HOSTIF_TX_TYPE_PIPELINE_LOOKUP
            
            sai_thrift_send_hostif_packet(self.client, rif_id1, str(pkt1), oam_tx_type, host_if_tx_type, ingress_port=port2)
            
            self.ctc_show_packet(1, compare_pkt=str(exp_pkt1), is_cpu_tx=1)
...
class scenario_02_cpu_tx_bypass_test(sai_base_test.ThriftInterfaceDataPlane):
    def runTest(self):
        ...
        try:
            ...
            sai_thrift_send_hostif_packet(self.client, rif_id1, str(pkt1), oam_tx_type, host_if_tx_type, egress_port=port1)
            
            self.ctc_show_packet(1, compare_pkt=str(pkt1), is_cpu_tx=1)
